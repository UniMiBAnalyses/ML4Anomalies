...
dfSMQCD = ROOT.RDataFrame("SSWW_SM",path_to_ntuple+"/ntuple_SSWW_SM.root")
dfSMQCD = dfSMQCD.Filter(kinematicFilter)
# minimo sforzo: semplicemente prendo tails di SM e lo metto dentro qcd
dfBSM = ROOT.RDataFrame("SSWW_"+str(oper)+"_QU",path_to_ntuple+"/ntuple_SSWW_"+str(oper)+"_QU.root")
dfBSM_LI = ROOT.RDataFrame("SSWW_"+str(oper)+"_LI",path_to_ntuple+"/ntuple_SSWW_"+str(oper)+"_LI.root")
...
print ("shape pandas SM ", wpdSM.shape)
...
SM_only_train = t.transform(SM_only_train)
SM_only_test = t.transform(SM_only_test)
...
mc = tf.keras.callbacks.ModelCheckpoint('./models/best_model_SSWW_2SM_trained_on'+str(oper)+'_.h5', monitor='val_auc', mode='max', verbose=1, save_best_only=True)
...
nameExtenstion = "SSWW_2SM_trained_on_"+str(oper)+"_model_"+str(intermediate_dim)+"_"+str(input_dim)+"_"+str(half_input)+"_"+str(latent_dim)+"_"+str(epochs)+"_"+str(batchsize)

tf.keras.models.save_model(encoder,'./models/encoder_'+nameExtenstion)
tf.keras.models.save_model(vae,'./models/vae_'+nameExtenstion)
tf.keras.models.save_model(outputLoss,'./models/outputLoss_'+nameExtenstion)
...
vae.load_weights('./models/best_model_SSWW_2SM_trained_on'+str(oper)+'_.h5')
